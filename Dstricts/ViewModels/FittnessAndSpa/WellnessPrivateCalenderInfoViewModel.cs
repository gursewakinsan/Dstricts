...
{
	public class WellnessPrivateCalenderInfoViewModel : BaseViewModel
	{
		#region Constructor.
		public WellnessPrivateCalenderInfoViewModel(INavigation navigation)
		{
		...
		private async Task ExecuteSelectEmployeeForSelectedServicesCommand()
		{
			...
			if (SelectEmployeeForSelectedServicesList?.Count > 0)
			{
				EmployeeId = SelectEmployeeForSelectedServicesList[0].Id;
				SelectEmployeeForSelectedServicesList[0].UserImageBorderColor = Color.FromHex("#2A2A31");
			}
		...
		private async Task ExecuteEmployeeBookingCalenderInfoAppCommand()
		{
			DependencyService.Get<IProgressBar>().Show();
			IFittnessAndSpaService service = new FittnessAndSpaService();
			var response = await service.WellnessPrivateCalenderInfoAppAsync(new Models.EmployeeBookingCalenderInfoAppRequest()
			{
				CheckId = Helper.Helper.HotelCheckedIn,
				DstrictsUserId = Helper.Helper.LoggedInUserId,
				WellnessId = Helper.Helper.WellnessId,
