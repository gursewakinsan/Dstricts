...
{
	public class VerifyUserInvitationInfoCodeViewModel : BaseViewModel
	{
		#region Constructor.
		public VerifyUserInvitationInfoCodeViewModel(INavigation navigation)
		{
		...
		private async Task ExecuteVerifySmsPasswordCommand()
		{
			...
			else
			{
				...
				else if (!Password.Equals(VerifyUserInvitationInfoCode.ToString()))
					await Helper.Alert.DisplayAlert("You have entered wrong code.");
				else
				{
					if (Device.RuntimePlatform == Device.iOS)
						await Launcher.OpenAsync($"QloudidUrl://DstrictsApp/ConfirmUserInvitationInfo/{Helper.Helper.VerifyUserInvitationInfoId}");
					else
						await Launcher.OpenAsync($"https://qloudid.com/ip/DstrictsApp/ConfirmUserInvitationInfo/{Helper.Helper.VerifyUserInvitationInfoId}");
				}
			}
		}
		#endregion
		...
		public int VerifyUserInvitationInfoCode { get; set; }
		#endregion
	}
}
